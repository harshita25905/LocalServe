  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div
          ref={suggestionsRef}
          className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-md shadow-lg z-50 max-h-60 overflow-y-auto animate-fade-in-down"
        >
          {query.length === 0 && showPopular && (
            <div className="px-3 py-2 text-xs font-medium text-gray-500 border-b bg-gray-50">Popular Locations</div>
          )}
          {suggestions.map((location, index) => (
            ...
            >
              ...
              {location.popular && (
                <span className="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded transition-all duration-200 hover:bg-blue-200">
                  Popular
                </span>
              )}
            </button>
